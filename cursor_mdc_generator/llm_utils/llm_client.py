...
async def process_large_content(
    ...
    chunk_tasks = [
        generate_response(
            messages=[
                {"role": "system", "content": f"{system_prompt} You are processing part {i+1} of {len(chunks)} of a large codebase."},
    ...
    final_result = await generate_response(
        messages=[
            {"role": "system", "content": "You are an expert at synthesizing and combining documentation from multiple sources."},
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        tasks = [
            generate_mdc_response(
                p.get("system_prompt", "You are an expert code documentation specialist."),
                p["user_prompt"],
                model_name,
                temperature
            ) 
